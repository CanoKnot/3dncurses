...
int sixrgb(int r, int g, int b) {
	return (int)(6*(r*36 + g*6 + b)/256.);
}

int sixrgb2(int r, int g, int b) {
	return 36*r + 6*g + 1*b;
}
...
struct Vec3 {
	...
	double length;
	bool l_computed;
	Vec3(): x(0),y(0),z(0),length(0),l_computed(false) {}
	Vec3(double x0, double y0, double z0): x(x0), y(y0), z(z0), length(0), l_computed(false) {}
	Vec3(const Vec3& a): x(a.x), y(a.y), z(a.z), length(0), l_computed(false) {}
	Vec3(const Vec2& v0): x(v0.x), y(v0.y), z(0), length(0), l_computed(false) {}
	double l() const {
		if (l_computed) {
			return length;
		} else {
...
struct Texture {
	vector<vector<Color3d>> map;
	int height, width;
	Texture(const char* filename){
		//unsigned char* tex = new unsigned char[1024*1024];
		int h;
		int w;
		map = BMPLoad(filename, w, h);
		height = h; 
		width = w; 
/*
		int r,g,b,c;
...
struct Cube {
	...
	Cube(const Vec3& c0, const Texture& topt, const Texture& side):
		top(c0 + Vec3(0,-.5,0), Vec3(0,-1,0), topt, 20),
		bottom(c0 + Vec3(0,.5,0), Vec3(0,1,0), topt, 21),
		left(c0 + Vec3(-.5,0,0), Vec3(-1,0,0), side, 22),
		right(c0 + Vec3(.5,0,0), Vec3(1,0,0), side, 23),
		hub(c0 + Vec3(0,0,.5), Vec3(0,0,1), side, 24),
		rim(c0 + Vec3(0,0,-.5), Vec3(0,0,-1), side, 25),
		sides({top,bottom,right,left,hub,rim})
	{}
};
...
*/
	...
	double lightdir_len = lightdir.l();
	double lightdir_len3 = lightdir_len*lightdir_len*lightdir_len;
	double dot_l = light.intensity*dot(lightdir, plane.h)
						/ (lightdir_len3*plane.h.l() + EPS);
	if (dot_l < 0) {
		dot_l *= -1;
	} 
	double localC_l = localC.l();
 /* = light.intensity*dot(lightdir, plane.h)
					 ...
					 x <= min(a1[sk+1].x,1.*(COLS/2)+1); x++) 
			{
				...
				do {		
					...
					// b = epsdot_ch*dot(r,j) - dot_cj;
						//cerr << "a = " << a << ", b = " << b << endl;
					 	//cerr << "aa";
						Color3d tex_color = plane.texture.get_color(a,b);
						// cerr << "cc";
					//Vec3 r1 = transform*r;	
				...
				double AMBIENT_LIGHT = 0;
					double lolc = min(1.,max(2*atan(dot_l)/M_PI,AMBIENT_LIGHT));
					color = sixrgb2(lolc*tex_color.r,
						lolc*tex_color.g,
						lolc*tex_color.b);
					pix(x+COLS/2,y+LINES/2,color, localC_l);
					y++;				    
				}	
...
struct Map {
	...
	void add(int x, int y, int z, Texture top, Texture side) {
		if (x > x0 || x < 0 ||
			y > y0 || y < 0 || 
			z > z0 || z < 0) {
			return;
		}
		map[x][y][z] = new Cube(Vec3(x,y,z), top,side);
	}
};
...
for (int i = 0; i <= 215; i++) {
	init_color(i, 1000*(i/36)/6, 
		1000*(i/6%6)/6, 
		1000*(i%6)/6);
	init_pair(i, 0,i);
}
int background_color = sixrgb2(0,0,0);
/*
for (int i = 0; i <= 23; i++) {
	init_pair(232+i, COLOR_BLACK,232+i);
	attron(COLOR_PAIR(100+i));
	mvaddch(i,0,'@');
}
init_pair(256, COLOR_BLACK,16);
...
*/
	...
	Texture texture("tmp.bmp");
	Texture top("grass_block_top.bmp");
	Texture side("grass_block_side.bmp");
	Vec3 pos(-0,10,0);	
	...
	Map map(12,12,12);
		for (int x = 0; x < 12; x++) {
			for (int z = 0; z < 12; z++) {
				map.add(x,0,z,top,side);
			}
		}

		Cube box1(Vec3(0,0,5), top,side);
		Cube box2(Vec3(1,1,5), top,side);
		Plane ground(Vec3(-5,2,5), Vec3(0, -1,0), 5, 5,texture);

		vector<Cube> Box;
		for (int i = 1; i < 2; i++) {
			for (int j = 5; j < 6; j++) {
				Box.push_back(Cube(Vec3(j,i,2),top,side));
			}
		}
		Vec3 h(1,0,0);
